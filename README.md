...
The same account menu provides **Delete my Mood & Moments data**. This action requires the exact `DELETE MY DATA` confirmation and Clerk strict reverification on every attempt. The Next.js route verifies the Clerk session first, and the deletion RPCs independently fail closed unless the signed Clerk JWT contains a fresh `fva` claim matching Clerk's strict ten-minute semantics. It deletes only the signed-in owner's cloud Moments and private images; the Clerk account, browser-local legacy data, static examples, and operational rate-limit counters remain intact. Database deletion is committed before private Storage cleanup, and a durable owner-scoped job makes partial cleanup retryable. Moment creation and private image upload both serialize with deletion through the same owner lock, so deletion cannot report success while an in-flight write can later become visible. The API returns success only after PostgreSQL verifies that Moments, Storage objects, cleanup authorizations, and the deletion job are all absent. Apply `supabase/migrations/20260905090000_add_account_data_deletion.sql`, `supabase/migrations/20260905100000_enforce_account_data_deletion_reverification.sql`, and `supabase/migrations/20260905110000_lock_moment_image_uploads_during_account_deletion.sql` in order before enabling this action in a deployed environment.
